...
class UpsertReportGroupItemRequest extends FormRequest
{
    ...
    {
        $type = $this->input('index_type');

        return [
            'index_type' => ['required', Rule::enum(IndexType::class)],
    ...
    {
        return IndexType::from($this->string('index_type')->toString());
    }
    ...
    public function hasIndexReturnContext(): bool
    {
        return $this->filled('return_type') || $this->filled('return_scope') || $this->filled('return_report');
    }
    ...
    public function indexRouteParameters(IndexType $defaultType): array
    {
        ...
        $scope = IndexScope::tryFrom($this->string('return_scope')->toString()) ?? IndexScope::Report;
